...
MainWindow::MainWindow(QWidget *parent)
  ...
  ui->label_count_vert->setText(
      QString::number(ui->openGLWidget->obj->count_v));
  ui->label_name->setText(fileInfo.fileName());
  ui->label_count_edge->setText(QString::number(count_indices_for_polygon(ui->openGLWidget->obj)/4));
...
void MainWindow::load_model() {
  ...
  if (fileInfo.exists()) {
    switch_menu(true);
    ui->openGLWidget->obj =
        parse_obj_file(ui->path->text().toLocal8Bit().data());
    ui->openGLWidget->setting.setValue("Path", ui->path->text());
    ui->openGLWidget->vertices_array =
        (float *)malloc((ui->openGLWidget->obj->count_v * sizeof(float)) * 3);
    obj_vertices_to_1D_array(ui->openGLWidget->obj,
                             ui->openGLWidget->vertices_array);
    ui->openGLWidget->indices_array = (int *)calloc(
        count_indices_for_polygon(ui->openGLWidget->obj), sizeof(int));
    obj_polygons_to_1D_array(ui->openGLWidget->obj,
                             ui->openGLWidget->indices_array);
    ui->openGLWidget->toRemove = false;
    ui->openGLWidget->update();
    ui->label_count_vert->setText(
        QString::number(ui->openGLWidget->obj->count_v));
    ui->label_name->setText(fileInfo.fileName());
...
void MainWindow::remove() {
  if (!ui->openGLWidget->toRemove) {
    free_memory(ui->openGLWidget->obj, ui->openGLWidget->vertices_array,
                ui->openGLWidget->indices_array);
...
void MainWindow::make_scr() {
  ...
  QString filePath = QFileDialog::getSaveFileName(nullptr, "Сохранить скриншот",
                                                  "", "Images (*.jpeg *.bmp)");
  if (!filePath.isEmpty()) {
...
void MainWindow::rotate_x() {
  ...
  rotate(ui->openGLWidget->obj,
         ui->x_rorate_spin->value() - ui->openGLWidget->angles[X], X,
         ui->openGLWidget->vertices_array);
  ui->openGLWidget->angles[X] = ui->x_rorate_spin->value();
  ui->openGLWidget->update();
}
...
void MainWindow::rotate_y() {
  ...
  rotate(ui->openGLWidget->obj,
         ui->y_rorate_spin->value() - ui->openGLWidget->angles[Y], Y,
         ui->openGLWidget->vertices_array);
  ui->openGLWidget->angles[Y] = ui->y_rorate_spin->value();
  ui->openGLWidget->update();
}
...
void MainWindow::rotate_z() {
  ...
  rotate(ui->openGLWidget->obj,
         ui->z_rorate_spin->value() - ui->openGLWidget->angles[Z], Z,
         ui->openGLWidget->vertices_array);
  ui->openGLWidget->angles[Z] = ui->z_rorate_spin->value();
  ui->openGLWidget->update();
}
...
void MainWindow::move_x() {
  move_obj(ui->openGLWidget->obj, ui->openGLWidget->vertices_array, X,
           (ui->x_pos_slider->value() - ui->openGLWidget->move_pos[X]) / 100);
  ui->openGLWidget->move_pos[X] = ui->x_pos_slider->value();
  ui->openGLWidget->update();
}

void MainWindow::move_y() {
  move_obj(ui->openGLWidget->obj, ui->openGLWidget->vertices_array, Y,
           (ui->y_pos_slider->value() - ui->openGLWidget->move_pos[Y]) / 100);
  ui->openGLWidget->move_pos[Y] = ui->y_pos_slider->value();
  ui->openGLWidget->update();
}

void MainWindow::move_z() {
  move_obj(ui->openGLWidget->obj, ui->openGLWidget->vertices_array, Z,
           (ui->z_pos_slider->value() - ui->openGLWidget->move_pos[Z]) / 100);
  ui->openGLWidget->move_pos[Z] = ui->z_pos_slider->value();
  ui->openGLWidget->update();
}

void MainWindow::scale_obj() {
  scale(ui->openGLWidget->obj, ui->openGLWidget->vertices_array,
        ui->scale_spin->value() / ui->openGLWidget->scale_value / 100);
  ui->openGLWidget->scale_value = ui->scale_spin->value() / 100;
  ui->openGLWidget->update();
}
